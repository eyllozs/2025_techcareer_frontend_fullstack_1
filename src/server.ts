...
const databaseCloudUrl =
  "mongodb+srv://eylulozis:<db_password>@offlinenodejscluster.ayhy3.mongodb.net/?retryWrites=true&w=majority&appName=offlinenodejscluster";
...
app.post("/register/api", csrfProtection, (request:any, response:any) => {
  ...
  newRegister
      ...
      .then(() => {
          ...
          response.send("CSRF ile regsiter başarıyla kaydedildi.");
      })
      .catch((err:any) => {
          console.log("Veritabanı hatası:", err);
          logger.error("Veritabanı hatası:", err); //logger: Winston
